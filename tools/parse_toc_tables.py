...
def parse_toc_tables(pdf_path, start_page, end_page, output_dir):
    """
    Parse tables list from PDF pages to extract table catalog.
    ...
    """
    print_section_header("TOC TABLES PARSING")
    ...
    # Extract tables list pages to temporary PDF
    import tempfile
    with tempfile.TemporaryDirectory() as temp_dir:
        tables_pdf_path = Path(temp_dir) / "toc_tables.pdf"
        
        print_progress(f"Extracting pages {start_page}-{end_page}...")
        success = extract_pages_to_pdf(pdf_path, str(tables_pdf_path), start_page, end_page)
        if not success:
            print_progress("- Failed to extract tables list pages")
            return False
        
        # Convert to images
        print_progress("Converting PDF to images...")
        image_paths = pdf_to_images(str(tables_pdf_path), temp_dir)
        if not image_paths:
        ...
        yaml_structure = create_tables_yaml_structure()
        prompt = create_toc_parsing_prompt("tables", yaml_structure)
        
        # Add text guidance to prompt
        if text_context:
            prompt += f"\n\nEXTRACTED TEXT FOR REFERENCE:\n{text_context[:2000]}..."
        
        print_progress("Sending to GPT-4 Vision API for tables extraction...")
        ...
        if result and not result.startswith("Error:"):
            ...
            # Save YAML output
            yaml_output_path = output_path / "thesis_tables.yaml"
            with open(yaml_output_path, 'w', encoding='utf-8') as f:
                f.write(cleaned_result.strip())
            
            # Save text output for reference
            text_output_path = output_path / "thesis_tables.txt"
            with open(text_output_path, 'w', encoding='utf-8') as f:
                f.write(f"Tables List Extraction\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Example:
  python parse_toc_tables.py thesis.pdf 17 17 structure/
  
This will extract the table catalog from page 17 and save it to structure/thesis_tables.yaml
"""
    )
